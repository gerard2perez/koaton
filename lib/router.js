...
function* restify(modelinstance, modelname) {
	const relation = require("../models/" + modelname)({}, {
		hasMany(field) {
...
function makeRestModel(options,mount_route,modelname) {
	console.log(mount_route);
	getrouter(options.get).get(`${mount_route}`, function*() {
			let limit, skip,
				res = {},
				where = {},
				filterset = {},
			...
			res[modelname] = yield this.model.mongooseFilter(mongoose);
			this.body = res;
		})
		.get(`${mount_route}:id`, function*() {
			let response = {};
			res[modelname] = yield restify(yield this.model.findById(this.params.id));
			this.body = response;
		});
	getrouter(options.post).post(`${mount_route}`, function*() {
		let response = {};
		res[modelname] = yield this.model.create(this.request.body[app.inflect.singularize(controller.Name)]);
		this.body = response;
	});
	getrouter(options.put).put(`${mount_route}:id`, function*() {
		...
		record.save();
		this.body = {};
		this.body[app.inflect.singularize(modelname)] = record;
	});
	getrouter(options.delete).del(`${mount_route}:id`, function*() {
...
module.exports.initialize = function(app) {
	...
	router = app.routers.www.public;
	secured = app.routers.www.protected;
	...
	Object.keys(apps).forEach((emberapp) => {
		const serveapp = function*(next) {
			yield next;
			if(!this.body){
				yield this.render(`/ember_apps/${apps[emberapp].directory}`);
			}
