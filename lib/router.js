...
function* findmodel(next) {
	let pmodel = this.request.url.split('?')[0].split('/')[1];
	this.model = this.db[pmodel];
	yield next;
}
...
function makeRestModel(options, route, modelname) {
	...
	pOrp(routers, options.get).get('/', function* REST_GET(next) {
		...
		let rawmodels = yield this.model.rawWhere(filterset, filteroptions);
		for (const idx in rawmodels) {
			rawmodels[idx] = yield restify(rawmodels[idx], modelname);
		}
	...
	pOrp(routers, options.post).post('/', function* REST_POST(next) {
		let res = {};
		console.log(1, this.request.body);
...
function initialize() {
	...
	const serveapp = function (directory) {
		return function* serveEmberAPP(next) {
			yield next;
			if (!this.body) {
