...
const setUpHandlebarsLayouts=()=>{
	...
	const templatesDir = join(process.cwd(), "views","layouts");
	...
	readDir("koaton_modules").forEach((folder)=>{
		readDir(join("koaton_modules",folder,"views","layouts")).forEach((file)=>{
...
module.exports = () => {
	...
	if(require(join(process.cwd(),"package.json")).dependencies.handlebars!==undefined){
		setUpHandlebarsLayouts();
	}
