...
const fs = require('fs');
const promesifythem = ['exists', 'create', 'findOrCreate', 'findOne', 'findById', 'find', 'all', 'run', 'updateOrCreate', 'upsert', 'update', 'remove', 'destroyById', 'destroy'
	//, 'destroyAll'
];
...
const addmodel = function(model_name, definition) {
	...
	model.rawAPI.destroyAll = model.destroyAll.bind(model);
	model.destroyAll = function() {
		return new Promise(function(resolve, reject) {
			model.rawAPI.destroyAll(function(err){
				/*console.log(err);
				if(err){
					// reject(err);
				}else{
...
module.exports.initialize = function(app) {
	...
	for (let model in relations) {
		relations[model].forEach((relation) => {
			let options = {
				as: relation.As,
	...
	co(function*() {
		...
		for (let index in files) {
			...
			try {
				...
				yield require(`${process.cwd()}/seeds/${file}`)(databases[app.inflect.pluralize(file.toLowerCase())]);
			} catch (err) {
