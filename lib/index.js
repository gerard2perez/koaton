...
const logger = require(process.cwd() + '/node_modules/koa-logger');
...
app.start = function(port, cb) {
    app.listen(port, function() {
        if (!(process.env.welcome === 'false') && config.env === "development") {
            ...
            console.log(`   Server running in ${process.env.PWD}\n` +
                `   To see your app, visit ` + `http://localhost:${config.port}\n`.underline +
                `   To shut down Koaton, press <CTRL> + C at any time.`);
            console.log("===".grey + "-----------------------------------------------------".dim + "===".grey);
            console.log('   Enviroment:\t\t' + (config.env).green);
