...
app.router = require('./router').initialize(app);
app.start = function(port) {
	fs.access("koaton_modules", fs.RF_OK | fs.W_OK, (err) => {
		if (!err) {
			readDir("koaton_modules").forEach((Module) => {
				console.log(path.join(process.cwd(), "koaton_modules", Module, "app.js"));
				try {
					require(path.join(process.cwd(), "koaton_modules", Module, "app.js"))(app);
				} catch (e) {}
			});
		}
		app.listen(port, () => {
