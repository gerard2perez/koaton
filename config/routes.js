...
module.exports = function (app) {
    let router = Router();
    let controllersDir = __dirname + "/../controllers/";
    fs.readdirSync(controllersDir).forEach(function (file) {
        ...
        if (controller.REST !== false) {
            ...
            app.use(function *(next){
                this.model = this.database[controller.Name];
                yield next;
            });
            router.get(`/${controller.Name}/`, function* (next) {
                ...
                this.body =  yield this.model.findOne({id:this.params.id});
                yield next;
            ...
            .del(`/${controller.Name}/:id`, function* (next) {
                yield this.model.destroy({id:this.params.id});
